...
{
    const uint16_t scr_reg = data->scr.get();
    ...
    for(uint16_t cur_reg = R0; cur_reg <= R3; ++cur_reg)
    {
        ...
        {
            dest[cur_reg] = data->reg[cur_reg - (cur_reg > scr_reg ? 1 : 0)].get(); //if reg > scratch reg then move array index back one to compensate
        }
    }
}
...
{
    ets_printf("D (%u) %s: BP:\t%s%u\t\t%s%u\t\t%s%u\t\t%s%u\t(PC: %u, Lab: %u%s, Line: %u)\n",
                esp_log_timestamp(),
                TAG,
...
static void hulp_debug_isr_handler(void* ctx)
{
    ...
    if( !(handle->data->marker.updated()) )
    {
        //No breakpoint data pending. Must be non-breakpoint wake (ie. normal user program I_WAKE). Ignore.
        return;
    }

    handle->data->marker.clearUpdated();

    hulp_debug_bp_cb_data_t bp_data = {0};
    ...
    bp_data.meta.reg_scr = handle->data->scr.get();
    ...
    bp_data.bp.line = handle->data->marker.get();
